...
float4 main(vs_out input) : SV_TARGET
{
	...
	{
		surfacePoint.material.color.w = 0.0;
	}
	...
	if (g_useIBL)
	{
		...
		if (g_useSpecularReflection)
		{
			...
			float3 specularIBL = g_specularIBL.SampleLevel(g_bilinearWrap, r, mipLevel) * (surfacePoint.material.F0 * factorIBL.r + factorIBL.g);
	...
	float alpha = surfacePoint.material.color.w;
	float threshold = 0.2;
	float falloff = 0.05;
